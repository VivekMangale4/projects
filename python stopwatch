...
class stopwatch(tk.Frame):
  ...
  def features(self):
    ...
    self.start_time_button=tk.Button(self,text="Start/Resume",height=5,width=7,font=('arial',19,'bold'),background='green',command=self.start_time)
    self.start_time_button.pack(side=tk.LEFT)

    self.stop_time_button=tk.Button(self,text='stop/Pause',height=5,width=7,font=('arial',19,'bold'),background='red',command=self.pause_time) 
    self.stop_time_button.pack(side=tk.LEFT)  

    self.reset_time_button=tk.Button(self,text='Reset',height=5,width=7,font=('arial',19,'bold'),background= 'yellow',command=self.result_time)
  ...
  def change(self):
      ...
      total_seconds_string=f'{self.total_seconds}' if self.total_seconds>9 else f'{self.total_seconds}'
